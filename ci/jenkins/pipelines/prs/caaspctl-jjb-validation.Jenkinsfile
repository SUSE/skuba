...
pipeline {
    ...
    stages {
        stage('Setting GitHub in-progress status') { steps {
            setBuildStatus('jenkins/caaspctl-jjb-validation', 'in-progress', 'pending')
        } }

        stage('Info') { steps {
            sh(script: "make -f caaspctl/ci/Makefile info", label: 'Info')
        } }
    ...
    post {
        ...
        failure {
            setBuildStatus('jenkins/caaspctl-jjb-validation', 'failed', 'failure')
        }
        success {
            setBuildStatus('jenkins/caaspctl-jjb-validation', 'success', 'success')
        }
    }
}
