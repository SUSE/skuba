   ...
   agent { node { label "caasp-team-private-${worker_type} && e2e" } }
    ...
    stages {
        ...
        stage('Conformance Tests') {
            ...
            steps {
                sh(script: "ci/tasks/sonobuoy_e2e.py run --kubeconfig ${WORKSPACE}/test-cluster/admin.conf --sonobuoy-version ${SONOBUOY_VERSION} --e2e-focus='${SONOBUOY_FOCUS}' --mode=certified-conformance", label: 'Run Conformance')
