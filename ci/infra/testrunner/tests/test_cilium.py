...
def test_cillium(deployment, kubectl):
    ...
    tie_out = kubectl.run_kubectl("exec tiefighter -- {}".format(landing_req)).stdout
    assert 'Ship landed' in tie_out

    xwing_out = kubectl.run_kubectl("exec xwing -- {} 2>&1 || :".format(landing_req)).stdout
    assert 'terminated with exit code 28' in xwing_out

    logger.info("Check status (N/N)")
    node_list = kubectl.run_kubectl("get nodes -o jsonpath='{ .items[*].metadata.name }'").stdout
    node_count = len(node_list.split(" "))
    cilium_podlist = kubectl.run_kubectl("get pods -n kube-system -l k8s-app=cilium -o jsonpath='{ .items[0].metadata.name }'").stdout.split(" ")
    cilium_podid = cilium_podlist[0]
    cilium_status_cmd = "-n kube-system exec {} -- cilium status".format(cilium_podid)
    cilium_status = kubectl.run_kubectl(cilium_status_cmd).stdout
