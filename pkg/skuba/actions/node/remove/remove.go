...
//
func Remove(clientSet k8s.Interface, target string, drainTimeout time.Duration) error {
	node, err := clientSet.CoreV1().Nodes().Get(target, metav1.GetOptions{})
	if err != nil {
	...
	if isControlPlane {
		...
		etcd.RemoveMember(clientSet, node)
	}

	if err := kubernetes.DisarmKubelet(clientSet, node); err != nil {
		fmt.Printf("[remove-node] failed disarming kubelet: %v; node could be down, continuing with node removal...\n", err)
	}

	if isControlPlane {
		if err := kubeadm.RemoveAPIEndpointFromConfigMap(clientSet, node); err != nil {
		...
		if err := cni.AnnotateCiliumDaemonsetWithCurrentTimestamp(clientSet); err != nil {
			fmt.Printf("[remove-node] could not annonate cilium daemonset: %v\n", err)
		}
	}

	if err := clientSet.CoreV1().Nodes().Delete(targetName, &metav1.DeleteOptions{}); err != nil {
